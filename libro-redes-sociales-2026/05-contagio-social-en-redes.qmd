...
La definición canónica es la de Centola & Macy (2007): un contagio es **simple** cuando basta *un* contacto con una fuente activada para transmitirlo (una enfermedad, una noticia, una oferta de trabajo), y es **complejo** cuando *"requiere contacto con múltiples fuentes de activación"* — es decir, cuando la exposición a un solo adoptante no basta y hace falta **refuerzo social**. En el lenguaje de Valente: un contagio complejo es uno cuyo umbral es $\tau_i > 1/z_i$, o en conteo, $a_i \geq 2$.

Esa definición tiene bordes difusos, y conviene saberlo: (i) "múltiples fuentes" puede medirse en **conteo** (dos vecinos) o en **fracción** (un cuarto de mis vecinos), y los resultados difieren; (ii) la literatura no siempre distingue **fuentes** (personas distintas) de **exposiciones** (veces que recibo la señal); (iii) el umbral puede ser homogéneo o distribuido en la población. Guilbeault, Becker & Centola (2018) revisan esas variantes; el punto común es que **los no-adoptantes cuentan**, y que un solo contacto nunca es suficiente.
...
- **Unirse a Facebook** (Ugander et al. 2012): predice la adopción el número de *grupos distintos* de amigos que ya están, no el número de amigos.
- **Cambiar la foto de perfil por el signo igual** en apoyo al matrimonio igualitario (State & Adamic 2015).
- **Una técnica agrícola nueva** en Malawi (Beaman et al. 2021): casi nadie adopta con un vecino informado; muchos, con dos.
- **Salir a protestar** en la Primavera Árabe (Steinert-Threlkeld 2017) y en las protestas de 2011–2013 (Barberá et al. 2015).
- **Una performance feminista** que saltó de Valparaíso a 60 países (Saetre 2025).
- Y en la teoría: la credibilidad de una leyenda urbana, la adopción de una tecnología nueva, la disposición a participar en una manifestación riesgosa, una moda de vanguardia (Centola & Macy 2007).
:::
...
1. **Complementariedad estratégica**: la innovación es costosa para los primeros y barata para los siguientes; se necesita masa crítica para que valga la pena.
2. **Credibilidad**: una innovación no es creíble hasta que varios vecinos la adoptan (los médicos y la tetraciclina).
3. **Legitimidad**: saber de un movimiento riesgoso no basta para unirse; ver a varios cercanos asistir, sí.
4. **Contagio emocional**: los impulsos expresivos se amplifican en reuniones concentradas social y espacialmente.

La idea que ordena los cuatro: una barrera es una razón por la que **una exposición no basta**. Y cada barrera tiene, en alguna disciplina, una teoría que explica *por qué* se necesitan varias fuentes. Ese es su microfundamento.

## Complementariedad estratégica

La innovación vale más cuantos más la usan: un teléfono, una red social, una moneda. Eso es, formalmente, un **juego de coordinación**: mi pago depende de que los demás elijan lo mismo que yo. Morris (2000) demostró que en una red ese juego produce exactamente un umbral: me conviene cambiar cuando la fracción de vecinos que ya cambió supera $q = b/(a+b)$, donde $a$ y $b$ son los pagos de coordinar en cada opción (véase también Easley & Kleinberg 2010, cap. 19). En economía, Katz & Shapiro (1985) formalizaron las **externalidades de red**: el valor del bien crece con su base instalada. Aquí "varias fuentes" es literal: necesito que *varios* de mis contactos usen el producto para que me sirva.

## Credibilidad

Cuando no sé si algo funciona, cada adoptante es evidencia. Bikhchandani, Hirshleifer & Welch (1992) y Banerjee (1992) lo modelaron bayesianamente como **cascadas informacionales**: si tres personas que eligieron por separado adoptaron, eso actualiza mi creencia más que una sola. Es la lógica de los médicos y la tetraciclina (Coleman, Katz & Menzel 1966): cada colega que la usa es una señal privada revelada. Y es donde vive el **efecto de fuentes múltiples** de Harkins & Petty (1981, 1987), que veremos en la sección siguiente: la información que llega de fuentes que no se coordinaron entre sí es más informativa que la misma información repetida.

## Legitimidad

Aquí no dudo de si funciona, sino de si *está permitido*. Bicchieri (2006) define una **norma social** como una regla que sigo porque espero que los demás la sigan *y* que esperen que yo la siga; por eso necesito ver a varios cumplirla para creer que rige. Samuelson & Zeckhauser (1988) agregan el freno del **sesgo de statu quo**: ante la duda, la gente se queda donde está. Ver a *varios* cercanos asistir a una protesta es lo que la legitima (Granovetter 1978; McAdam 1986 sobre el activismo de alto riesgo). Y, como veremos, aquí las fuentes deben ser además **diversas**: si solo mi grupo adopta, no sé si es legítimo *en general*.

## Contagio emocional

La barrera no es cognitiva sino afectiva: necesito el ánimo. Collins (2004) formaliza la *efervescencia colectiva* de Durkheim como **cadenas de rituales de interacción**: la energía emocional se genera en copresencia y se amplifica con el número de participantes. Hatfield, Cacioppo & Rapson (1993) documentan el **contagio emocional** automático: mimetismo facial, sincronía postural, convergencia afectiva. Aquí "varias fuentes" es casi físico: una persona entusiasmada no hace una marcha; una multitud sí.
...
En un contagio simple, los no-adoptantes son irrelevantes: si tengo contacto con alguien con sarampión, me contagio, sin importar cuántos sanos conozca. En un contagio complejo, los no-adoptantes **son una señal activa**: cada contacto que no adoptó dice, en silencio, "esto no es aceptado". Centola las llama *influencias contrarias* (*countervailing influences*).

Por lo tanto, lo que predice adopción no es cuántos amigos ya lo han hecho, sino **qué fracción**.

La consecuencia es la primera sorpresa del bloque: **cuanto más conectado estás, más influencias contrarias enfrentas**. Por eso los "influyentes" suelen ser *frenos* y no *aceleradores* de los contagios complejos. Y, por eso, tantos movimientos —la Primavera Árabe en Egipto (Steinert-Threlkeld 2017), la adopción de Twitter (Toole et al. 2012), el *Ice Bucket Challenge* (Sprague & House 2017)— **nacieron en la periferia**.

## Fuentes, no exposiciones

Lo que cuenta no es cuántas *veces* recibo la señal, sino de cuántas **fuentes distintas e independientes** la recibo.

**Harkins & Petty (1981, 1987)** hallaron este *efecto de fuentes múltiples*: los mismos argumentos, presentados por varias personas *independientes*, persuaden más que repetidos por una. 
...
Las mascarillas en la pandemia: la misma conducta fue contagio simple en comunidades que confiaban en la autoridad sanitaria y contagio complejo donde no.
...
Centola & Macy dicen que el colapso de la propagación al aumentar $p$ es "una transición de fase de primer orden": la saturación pasa de completa a ninguna **de golpe**, no gradualmente, y con histéresis. Qué significa eso, de dónde viene el término (el modelo de Ising) y por qué importa para los *tipping points* y la irreversibilidad de las normas está en el [anexo sobre transiciones de fase](anexos.qmd).
:::
...
![Camino simple vs. camino complejo. Entre $A$ y $B$ hay un camino simple corto (por el atajo), pero un contagio con $\tau = 2$ no puede recorrerlo: necesita avanzar por **puentes anchos**, pares de lazos que lleguen juntos a cada nodo nuevo. El camino complejo es más largo y pasa por otros nodos. Figura 1 de Guilbeault & Centola (2021).](figs/contagio/11-guilbeault-centola-camino-complejo.png){width="75%" fig-align="center"}
...
# 25 aldeas, 10 períodos (toa = 11 significa "no adoptó")
...
Tres gráficos vienen incluidos: la curva de adoptantes, el umbral observado de cada persona, y la red coloreada por período:

```{r}
#| label: fig-nd-kfamily-plots
#| fig-cap: "Aldea 21. Izquierda: adoptantes por período y acumulados. Derecha: umbral observado (fracción de contactos que ya habían adoptado) según el tiempo de adopción de cada mujer."
#| fig-height: 4
#| fig-width: 9
#| message: false
#| warning: false
...
donde $E_{i,t-1}$ es la fracción de contactos que ya habían adoptado en el período anterior. `diffreg()` lo estima directamente sobre el `diffnet`, pero exige que la exposición se llame literalmente `exposure`:
...
modelo <- diffreg(kfamily_diffnet_21 ~ exposure + factor(per), type = "logit")
round(coef(summary(modelo))["exposure", , drop = FALSE], 3)
```

La exposición es **positiva y significativa**: en la aldea 21, tener más contactos que ya usan planificación familiar aumenta la probabilidad de adoptarla al período siguiente, más allá de la tendencia temporal común. Eso es la firma mínima de un efecto de red.
...
1. Repite la regresión para toda `kfamily` (sin el `subset`) y para `brfarmersDiffNet`, que ya viene como `diffnet`. ¿Cambia el coeficiente?
2. Hazlo con `medInnovationsDiffNet`. Aquí la exposición **no** es significativa: es coherente con Van den Bulte & Lilien (2001), que mostraron que el "contagio" de la tetraciclina se desvanece al controlar por el marketing farmacéutico.
3. Agrega covariables de la encuesta al modelo (`media12`, exposición a afiches; `sons`; `pregs`). ¿Sobrevive la exposición?
:::
